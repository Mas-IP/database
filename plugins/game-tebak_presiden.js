...
const GAME_FILE = path.join(GAME_DIR, 'tebakpresiden.json')

const API_URL = `https://api.botcahx.eu.org/api/game/tebakpresiden?apikey=${btc}`
...
let handler = async (m, { conn, usedPrefix }) => {
    try {
        conn.tebakpresiden = conn.tebakpresiden
            ? conn.tebakpresiden
            : {}
...
`.trim()
        ...
        console.error('[TEBAKPRESIDEN ERROR]', e)

        await conn.reply(
            m.chat,
            '❌ Gagal menjalankan tebak presiden.',
            m
        )
    }
}
