//create code Wonge-bot

const free = 1000
const prem = 2000
const limitfree = 10
const limitprem = 20
const moneyfree = 1000000
const moneyprem = 3000000
const timeout = 604800000

let handler = async function (m, { conn, isPrems }) {
    let time = global.db.data.users[m.sender].lastweekly + timeout

    if (new Date() - global.db.data.users[m.sender].lastweekly < timeout) {
        throw 'Anda sudah mengklaim, klaim mingguan ini\ntunggu selama ' + msToTime(time - new Date()) + ' lagi'
    }

    global.db.data.users[m.sender].exp += isPrems ? prem : free
    global.db.data.users[m.sender].money += isPrems ? moneyprem : moneyfree
    global.db.data.users[m.sender].limit += isPrems ? limitprem : limitfree
    // global.db.data.users[m.sender].legendary += 3

    conn.reply(
        m.chat,
        'Selamat kamu mendapatkan:\n\n+' +
            (isPrems ? prem : free) +
            ' Exp\n+' +
            (isPrems ? moneyprem : moneyfree) +
            ' Money\n+' +
            (isPrems ? limitprem : limitfree) +
            ' Limit',
        m
    )

    global.db.data.users[m.sender].lastweekly = new Date().getTime()
}

handler.help = ['mingguan']
handler.tags = ['rpgabsen']
handler.command = /^(mingguan)$/i
handler.limit = false
handler.fail = null
handler.rpg = true

export default handler

function msToTime(duration) {
    var milliseconds = parseInt((duration % 1000) / 100)
    var seconds = Math.floor((duration / 1000) % 60)
    var minutes = Math.floor((duration / (1000 * 60)) % 60)
    var hours = Math.floor((duration / (1000 * 60 * 60)) % 24)
    var weeks = Math.floor((duration / (1000 * 60 * 60 * 24)) % 168)

    weeks = weeks < 10 ? '0' + weeks : weeks
    hours = hours < 10 ? '0' + hours : hours
    minutes = minutes < 10 ? '0' + minutes : minutes
    seconds = seconds < 10 ? '0' + seconds : seconds

    return weeks + ' hari ' + hours + ' jam ' + minutes + ' menit'
}